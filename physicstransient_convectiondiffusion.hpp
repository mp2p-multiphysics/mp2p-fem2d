...
class PhysicsTransientConvectionDiffusion : public PhysicsTransientBase
{
    ...
    */
    ...
    (
        ...
        ScalarGroup &derivativecoefficient_group_in, ScalarGroup &diffusioncoefficient_group_in, ScalarGroup &velocity_x_group_in, ScalarGroup &velocity_y_group_in, ScalarGroup &generationcoefficient_group_in
    )
    {
        ...
        scalar_group_ptr_vec = {derivativecoefficient_group_ptr, diffusioncoefficient_group_ptr, velocity_x_group_ptr, velocity_y_group_ptr, generationcoefficient_group_ptr};
...
)
{
    ...
    */
    ...
    for (int indx_d = 0; indx_d < domain_group_ptr->domain_t3_ptr_vec.size(); indx_d++)
    {
        ...
        // subset the scalars
        ScalarTri3 *derivativecoefficient_ptr = derivativecoefficient_group_ptr->scalar_t3_ptr_vec[indx_d];
    ...
    for (int indx_d = 0; indx_d < domain_group_ptr->domain_q4_ptr_vec.size(); indx_d++)
    {
        ...
        // subset the scalars
        ScalarQuad4 *derivativecoefficient_ptr = derivativecoefficient_group_ptr->scalar_q4_ptr_vec[indx_d];
    ...
    for (int indx_b = 0; indx_b < boundary_group_ptr->boundary_t3_ptr_vec.size(); indx_b++)
    {
        ...
        // subset the scalars
        ScalarTri3 *derivativecoefficient_ptr = derivativecoefficient_group_ptr->scalar_t3_ptr_vec[indx_b];
    ...
    for (int indx_b = 0; indx_b < boundary_group_ptr->boundary_q4_ptr_vec.size(); indx_b++)
    {
        ...
        // subset the scalars
        ScalarQuad4 *derivativecoefficient_ptr = derivativecoefficient_group_ptr->scalar_q4_ptr_vec[indx_b];
...
)
{
    ...
    for (int edid = 0; edid < domain_ptr->num_element; edid++)
    {
        ...
        double dervcoeff_p2 = derivativecoefficient_ptr->point_value_vec[p2_pdid];
...
)
{
    ...
    for (int bnid = 0; bnid < boundary_ptr->num_natural; bnid++)
    {
        ...
        {
            case 1:  // neumann
                d_vec.coeffRef(mat_row_pa) += pa_parameter_vec[0] * integral_ptr->integral_boundary_Ni_vec[edid][boundary_key][pa_plid];
                d_vec.coeffRef(mat_row_pb) += pb_parameter_vec[0] * integral_ptr->integral_boundary_Ni_vec[edid][boundary_key][pb_plid];
            break;
            case 2:  // robin
                d_vec.coeffRef(mat_row_pa) += pa_parameter_vec[0] * integral_ptr->integral_boundary_Ni_vec[edid][boundary_key][pa_plid];
                d_vec.coeffRef(mat_row_pb) += pb_parameter_vec[0] * integral_ptr->integral_boundary_Ni_vec[edid][boundary_key][pb_plid];
...
)
{
    ...
    for (int beid = 0; beid < boundary_ptr->num_essential; beid++)
    {
        ...
        c_mat.row(mat_row_pb) *= 0.;
        d_vec.coeffRef(mat_row_pa) = 0.;
        d_vec.coeffRef(mat_row_pb) = 0.;
    ...
    for (int beid = 0; beid < boundary_ptr->num_essential; beid++)
    {
        ...
        switch (btid)
        {
            case 0:  // dirichlet
                ...
                d_vec.coeffRef(mat_row_pa) += pa_parameter_vec[0];
                d_vec.coeffRef(mat_row_pb) += pb_parameter_vec[0];
            break;
        }
...
)
{
    ...
    for (int edid = 0; edid < domain_ptr->num_element; edid++)
    {
        ...
        double dervcoeff_p1 = derivativecoefficient_ptr->point_value_vec[p1_pdid];
        double dervcoeff_p2 = derivativecoefficient_ptr->point_value_vec[p2_pdid];
        double dervcoeff_p3 = derivativecoefficient_ptr->point_value_vec[p3_pdid];
