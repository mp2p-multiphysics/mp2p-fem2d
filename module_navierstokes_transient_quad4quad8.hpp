#ifndef MODULE_NAVIERSTOKES_TRANSIENT_QUAD4QUAD8
#define MODULE_NAVIERSTOKES_TRANSIENT_QUAD4QUAD8
#include <vector>
#include "Eigen/Eigen"
...
struct NavierStokesTransientQuad4Quad8IntegralStruct
{
...
class NavierStokesTransientQuad4Quad8Class
{
    ...
    void matrix_fill(
        ...
        double dt, int start_id_u, int start_id_v, int start_id_p
    );

    // struct with integrals
    NavierStokesTransientQuad4Quad8IntegralStruct nstq4q8is;

    // constructor
    NavierStokesTransientQuad4Quad8Class(GridQuad8Struct &gq8s_in, GridQuad4Struct &gq4s_in, BoundaryQuad8Struct &u_bq8s_in, BoundaryQuad8Struct &v_bq8s_in, BoundaryQuad4Struct &p_bq4s_in)
    {
        gq8s = gq8s_in;
        gq4s = gq4s_in;
        u_bq8s = u_bq8s_in;
...
void NavierStokesTransientQuad4Quad8Class::integral_evaluate()
{
    ...
    for (int m = 0; m < gq4s.num_element; m++)
    {
        ...
        nstq4q8is.integral_Mi_quad8_vec.push_back(integral_Mi_quad8_part_i_vec);
        nstq4q8is.integral_Ni_quad4_derivative_Mj_quad8_x_vec.push_back(integral_Ni_quad4_derivative_Mj_quad8_x_part_i_vec);
        nstq4q8is.integral_Ni_quad4_derivative_Mj_quad8_y_vec.push_back(integral_Ni_quad4_derivative_Mj_quad8_y_part_i_vec);
        nstq4q8is.integral_Mi_quad8_derivative_Nj_quad4_x_vec.push_back(integral_Mi_quad8_derivative_Nj_quad4_x_part_i_vec);
        nstq4q8is.integral_Mi_quad8_derivative_Nj_quad4_y_vec.push_back(integral_Mi_quad8_derivative_Nj_quad4_y_part_i_vec);
        nstq4q8is.integral_div_Mi_quad8_dot_div_Mj_quad8_vec.push_back(integral_div_Mi_quad8_dot_div_Mj_quad8_part_i_vec);
        nstq4q8is.integral_Mi_quad8_Mj_quad8_vec.push_back(integral_Mi_quad8_Mj_quad8_part_i_vec);
        nstq4q8is.integral_Mi_quad8_Mk_quad8_derivative_Mj_quad8_x_vec.push_back(integral_Mi_quad8_Mk_quad8_derivative_Mj_quad8_x_part_i_vec);
        nstq4q8is.integral_Mi_quad8_Mk_quad8_derivative_Mj_quad8_y_vec.push_back(integral_Mi_quad8_Mk_quad8_derivative_Mj_quad8_y_part_i_vec);

    }

}

void NavierStokesTransientQuad4Quad8Class::matrix_fill
(
    Eigen::SparseMatrix<double> &a_mat, Eigen::VectorXd &b_vec, Eigen::VectorXd &x_last_iteration_vec, Eigen::VectorXd &x_last_timestep_vec,
    ScalarQuad8Class &density_sq8c, ScalarQuad8Class &viscosity_sq8c, double gravity_x, double gravity_y,
    double dt, int start_id_u, int start_id_v, int start_id_p
)
{
    ...
    int num_point_all = 2*gq8s.num_point + gq4s.num_point;
    Eigen::SparseMatrix<double> help_mat(num_point_all, num_point_all);  // helper matrix used to make b_vec
    ...
    for (int m = 0; m < gq8s.num_element; m++)
    {
        ...
        double u0_it_quad8 = x_last_iteration_vec[start_id_u + n0_quad8];
        double u1_it_quad8 = x_last_iteration_vec[start_id_u + n1_quad8];
        double u2_it_quad8 = x_last_iteration_vec[start_id_u + n2_quad8];
        double u3_it_quad8 = x_last_iteration_vec[start_id_u + n3_quad8];
        double u4_it_quad8 = x_last_iteration_vec[start_id_u + n4_quad8];
        double u5_it_quad8 = x_last_iteration_vec[start_id_u + n5_quad8];
        double u6_it_quad8 = x_last_iteration_vec[start_id_u + n6_quad8];
        double u7_it_quad8 = x_last_iteration_vec[start_id_u + n7_quad8];
        double u_it_quad8_arr[8] = {u0_it_quad8, u1_it_quad8, u2_it_quad8, u3_it_quad8, u4_it_quad8, u5_it_quad8, u6_it_quad8, u7_it_quad8};

        // get v velocity values at points around element (last iteration)
        double v0_it_quad8 = x_last_iteration_vec[start_id_v + n0_quad8];
        double v1_it_quad8 = x_last_iteration_vec[start_id_v + n1_quad8];
        double v2_it_quad8 = x_last_iteration_vec[start_id_v + n2_quad8];
        double v3_it_quad8 = x_last_iteration_vec[start_id_v + n3_quad8];
        double v4_it_quad8 = x_last_iteration_vec[start_id_v + n4_quad8];
        double v5_it_quad8 = x_last_iteration_vec[start_id_v + n5_quad8];
        double v6_it_quad8 = x_last_iteration_vec[start_id_v + n6_quad8];
        double v7_it_quad8 = x_last_iteration_vec[start_id_v + n7_quad8];
        double v_it_quad8_arr[8] = {v0_it_quad8, v1_it_quad8, v2_it_quad8, v3_it_quad8, v4_it_quad8, v5_it_quad8, v6_it_quad8, v7_it_quad8};

        // get u velocity values at points around element (last timestep)
        double u0_ts_quad8 = x_last_timestep_vec[start_id_u + n0_quad8];
        double u1_ts_quad8 = x_last_timestep_vec[start_id_u + n1_quad8];
        double u2_ts_quad8 = x_last_timestep_vec[start_id_u + n2_quad8];
        double u3_ts_quad8 = x_last_timestep_vec[start_id_u + n3_quad8];
        double u4_ts_quad8 = x_last_timestep_vec[start_id_u + n4_quad8];
        double u5_ts_quad8 = x_last_timestep_vec[start_id_u + n5_quad8];
        double u6_ts_quad8 = x_last_timestep_vec[start_id_u + n6_quad8];
        double u7_ts_quad8 = x_last_timestep_vec[start_id_u + n7_quad8];
        double u_ts_quad8_arr[8] = {u0_ts_quad8, u1_ts_quad8, u2_ts_quad8, u3_ts_quad8, u4_ts_quad8, u5_ts_quad8, u6_ts_quad8, u7_ts_quad8};

        // get v velocity values at points around element (last timestep)
        double v0_ts_quad8 = x_last_timestep_vec[start_id_v + n0_quad8];
        double v1_ts_quad8 = x_last_timestep_vec[start_id_v + n1_quad8];
        double v2_ts_quad8 = x_last_timestep_vec[start_id_v + n2_quad8];
        double v3_ts_quad8 = x_last_timestep_vec[start_id_v + n3_quad8];
        double v4_ts_quad8 = x_last_timestep_vec[start_id_v + n4_quad8];
        double v5_ts_quad8 = x_last_timestep_vec[start_id_v + n5_quad8];
        double v6_ts_quad8 = x_last_timestep_vec[start_id_v + n6_quad8];
        double v7_ts_quad8 = x_last_timestep_vec[start_id_v + n7_quad8];
        double v_ts_quad8_arr[8] = {v0_ts_quad8, v1_ts_quad8, v2_ts_quad8, v3_ts_quad8, v4_ts_quad8, v5_ts_quad8, v6_ts_quad8, v7_ts_quad8};
        ...
        for (int j = 0; j < 8; j++) {
            ...
            double integral_Mi_quad8_u_it_derivative_Mj_quad8_x = 0.;
            double integral_Mi_quad8_v_it_derivative_Mj_quad8_y = 0.;
            double integral_Mi_quad8_u_ts_derivative_Mj_quad8_x = 0.;
            double integral_Mi_quad8_v_ts_derivative_Mj_quad8_y = 0.;
            for (int k = 0; k < 8; k++)
            {
                integral_Mi_quad8_u_it_derivative_Mj_quad8_x += u_it_quad8_arr[k] * nstq4q8is.integral_Mi_quad8_Mk_quad8_derivative_Mj_quad8_x_vec[m][i][j][k];
                integral_Mi_quad8_v_it_derivative_Mj_quad8_y += v_it_quad8_arr[k] * nstq4q8is.integral_Mi_quad8_Mk_quad8_derivative_Mj_quad8_y_vec[m][i][j][k];
                integral_Mi_quad8_u_ts_derivative_Mj_quad8_x += u_ts_quad8_arr[k] * nstq4q8is.integral_Mi_quad8_Mk_quad8_derivative_Mj_quad8_x_vec[m][i][j][k];
                integral_Mi_quad8_v_ts_derivative_Mj_quad8_y += v_ts_quad8_arr[k] * nstq4q8is.integral_Mi_quad8_Mk_quad8_derivative_Mj_quad8_y_vec[m][i][j][k];
            }

            // calculate a_mat coefficients
            a_mat.coeffRef(start_id_u + n_quad8_arr[i], start_id_u + n_quad8_arr[j]) += (
                  inverse_dt*density_quad8_arr[i]*nstq4q8is.integral_Mi_quad8_Mj_quad8_vec[m][i][j]
                + 0.5*density_quad8_arr[i]*integral_Mi_quad8_u_it_derivative_Mj_quad8_x
                + 0.5*density_quad8_arr[i]*integral_Mi_quad8_v_it_derivative_Mj_quad8_y
                + 0.5*viscosity_quad8_arr[i]*nstq4q8is.integral_div_Mi_quad8_dot_div_Mj_quad8_vec[m][i][j]
            );

            // calculate help_mat coefficients
            help_mat.coeffRef(n_quad8_arr[i], n_quad8_arr[j]) += (
                  inverse_dt*density_quad8_arr[i]*nstq4q8is.integral_Mi_quad8_Mj_quad8_vec[m][i][j]
                - 0.5*density_quad8_arr[i]*integral_Mi_quad8_u_ts_derivative_Mj_quad8_x
                - 0.5*density_quad8_arr[i]*integral_Mi_quad8_v_ts_derivative_Mj_quad8_y
                - 0.5*viscosity_quad8_arr[i]*nstq4q8is.integral_div_Mi_quad8_dot_div_Mj_quad8_vec[m][i][j]
            );
        ...
        for (int i = 0; i < 8; i++) {
        for (int j = 0; j < 4; j++) {
            a_mat.coeffRef(start_id_u + n_quad8_arr[i], start_id_p + n_quad4_arr[j]) += 0.5*nstq4q8is.integral_Mi_quad8_derivative_Nj_quad4_x_vec[m][i][j];
            help_mat.coeffRef(n_quad8_arr[i], 2*gq8s.num_point + n_quad4_arr[j]) += -0.5*nstq4q8is.integral_Mi_quad8_derivative_Nj_quad4_x_vec[m][i][j];
        }}

        // calculate b_vec coefficients
        for (int i = 0; i < 8; i++) {
            b_vec.coeffRef(start_id_u + n_quad8_arr[i]) += density_quad8_arr[i]*gravity_x*nstq4q8is.integral_Mi_quad8_vec[m][i];
        }
        ...
        for (int j = 0; j < 8; j++) {
            ...
            // calculate help_mat coefficients
            help_mat.coeffRef(gq8s.num_point + n_quad8_arr[i], gq8s.num_point + n_quad8_arr[j]) += (
                  inverse_dt*density_quad8_arr[i]*nstq4q8is.integral_Mi_quad8_Mj_quad8_vec[m][i][j]
                - 0.5*density_quad8_arr[i]*integral_Mi_quad8_u_ts_derivative_Mj_quad8_x
                - 0.5*density_quad8_arr[i]*integral_Mi_quad8_v_ts_derivative_Mj_quad8_y
                - 0.5*viscosity_quad8_arr[i]*nstq4q8is.integral_div_Mi_quad8_dot_div_Mj_quad8_vec[m][i][j]
        ...
        for (int i = 0; i < 8; i++) {
        for (int j = 0; j < 4; j++) {
            a_mat.coeffRef(start_id_v + n_quad8_arr[i], start_id_p + n_quad4_arr[j]) += 0.5*nstq4q8is.integral_Mi_quad8_derivative_Nj_quad4_y_vec[m][i][j];
            help_mat.coeffRef(gq8s.num_point + n_quad8_arr[i], 2*gq8s.num_point + n_quad4_arr[j]) += -0.5*nstq4q8is.integral_Mi_quad8_derivative_Nj_quad4_y_vec[m][i][j];
        }}

        // calculate b_vec coefficients
        for (int i = 0; i < 8; i++) {
            b_vec.coeffRef(start_id_v + n_quad8_arr[i]) += density_quad8_arr[i]*gravity_y*nstq4q8is.integral_Mi_quad8_vec[m][i];
        }
    ...
    for (int m = 0; m < gq4s.num_element; m++)
    {
        ...
        for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 8; j++) {
            a_mat.coeffRef(start_id_p + n_quad4_arr[i], start_id_u + n_quad8_arr[j]) += nstq4q8is.integral_Ni_quad4_derivative_Mj_quad8_x_vec[m][i][j];
        }}

        // coefficients of v
        for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 8; j++) {
            a_mat.coeffRef(start_id_p + n_quad4_arr[i], start_id_v + n_quad8_arr[j]) += nstq4q8is.integral_Ni_quad4_derivative_Mj_quad8_y_vec[m][i][j];
        }}
    ...
    b_vec += help_mat*x_last_timestep_vec(Eigen::seqN(start_id_u, num_point_all));
